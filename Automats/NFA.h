...
class NFA : public IMachine 
{
	...
	public:
		NFA() = default;
		NFA(std::map<Key, std::vector<std::string>> _table, \
		...
		DFSA AlgorithmTompson()
		{
			std::queue<std::set<std::string>> queueStates;
			std::set<std::string> queueNewStates;
			...
			return DFSA(newTable, queueNewStates, symbols, accessNewStates, wordVector);
		}
};
